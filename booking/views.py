...
class RoomFilter(filterss.FilterSet):
    min_price = filterss.NumberFilter(
        field_name="price_per_night",
        lookup_expr="gte",
        label=_("Minimum price per night"),
    )
    max_price = filterss.NumberFilter(
        field_name="price_per_night",
        lookup_expr="lte",
        label=_("Maximum price per night"),
    )
    check_in = filterss.DateFilter(
        method="filter_available",
        widget=DateInput(attrs={"type": "date"}),
        label=_("Check-in date"),
    )
    check_out = filterss.DateFilter(
        method="filter_available",
        widget=DateInput(attrs={"type": "date"}),
    ...
    def available(self, request):
        ...
        if check_in and check_out:
            ...
            if check_in >= check_out:
                return Response(
                    {"error": "check_in date must be before check_out date."}
                )
